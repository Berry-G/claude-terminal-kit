...
프로젝트마다 이 16색 중 하나가 배정되고, 배지와 게이지와 Windows Terminal 탭 색이 모두 같은 색을 씁니다.

![16색 팔레트](docs/palette.png)

색상환을 한 바퀴 돌면서 명도를 맞춰 고른 값들입니다. 배지는 **색을 배경으로 깔고 그 위에 어두운 글자**를
얹기 때문에, 어느 색이 걸려도 프로젝트명이 똑같이 읽히도록 열여섯 개가 비슷한 밝기에 있어야 합니다.
끝의 회색과 갈색은 나머지 열넷이 다 나간 뒤에도 서로 헷갈리지 않는 색이 필요해서 넣었습니다.

어느 색이 걸릴지는 프로젝트명 해시가 정하지만 거기서 끝이 아닙니다 — 열려 있는 창끼리 실시간으로
협상해서 같은 색이 겹치지 않게 비켜섭니다([자세히](#동작-원리)). 배정 결과는
`~\.claude\cc-colors.json` 에 남아서 다음에 열 때도 같은 색으로 나옵니다.
팔레트 자체를 바꾸고 싶으면 `cc-palette.ps1` 의 `$CCPalette` 한 곳만 고치면 됩니다.
